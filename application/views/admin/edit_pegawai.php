  ...
  <body>
    ...
    <div class="row no-gutters">
      ...
      <div class="col-md-10 p-5 pt-2">
        ...
        <form method="post" action="<?php echo base_url('admin/data_pegawai/edit_dataAksi') ?>">
          <div>
            <label>Nama</label>
            <input type="hidden" name="id_pegawai" class="form-control" value="<?php echo $peg->id_pegawai ?>">
            <input type="text" name="nama_pegawai" class="form-control" value="<?php echo $peg->nama_pegawai ?>">
          </div>
          <div>
            <label>Tempat Lahir</label>
            <input type="text" name="tempat_lahir" class="form-control" value="<?php echo $peg->tempat_lahir ?>">
          </div>
          <div>
            <label>Tanggal Lahir</label>
            <input type="date" name="tanggal_lahir" class="form-control" value="<?php echo $peg->tanggal_lahir ?>">
          </div>
          <div>
            <label>Bagian Kerja</label>
            <input type="text" name="bagian_kerja" class="form-control" value="<?php echo $peg->bagian_kerja ?>">
          </div>
          <div>
            <button type="submit" class="btn btn-success mt-3 btn-sm" ><i class="fa fa-save mr-1"></i>Simpan data</button>
            <a href="<?php echo base_url('admin/data_pegawai') ?>" class="btn btn-warning btn-sm mt-3" >Kembali</a>
          </div>
        </form>
      <?php endforeach; ?>
      </div>
